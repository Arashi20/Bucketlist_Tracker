"""Standalone read-only connector for the Bucketlist / travel planner app.

Deployed as its own Railway service pointing at this folder, sharing the main
app's Postgres database. It exposes the bucketlist, the scratch map and the
trip planner over MCP (for Claude custom connectors) and over plain REST, and
it never writes: no route accepts anything but GET or the MCP/OAuth
...
def create_app():
    ...
    def index():
        ...
        return jsonify({
            'service': 'bucketlist-connector',
            'access': 'read-only',
            'areas': ['bucketlist', 'visited countries', 'trips'],
            'mcp_endpoint': f'{base}/mcp',
            'tools': [tool['name'] for tool in TOOLS],
            'rest_endpoints': [f'{base}/api/v1/{name}' for name in
                               ('overview', 'bucketlist', 'countries', 'trips',
                                'ping')],
    ...
    def healthz():
        ...
        report = {
            ...
            # Whether the approval page can sign anyone in at all.
            'app_login_configured': bool(config.app_username() and config.app_password()),
            'dynamic_registration': config.dynamic_registration_enabled(),
        }
        warning = config.public_url_warning()
        if not report['app_login_configured']:
            warning = ('APP_USERNAME and APP_PASSWORD are not both set, so the '
                       'approval page cannot sign anyone in.')
        if warning:
    ...
    def whoami():
        ...
        if error is not None:
            return error
        return jsonify({'user': user, 'scope': 'read'})
