...
Users sign in with their normal app username and password - the same
APP_USERNAME / APP_PASSWORD variables the main app's login checks, since the
app keeps its single login in the environment rather than in a users table. Codes, tokens and client secrets are HMAC-signed values
...
import config
from auth import check_app_credentials
...
def issue_access_token(username, client_id):
    return _serializer('mcp-oauth-access').dumps(
        {'sub': username, 'client_id': client_id, 'scope': READ_SCOPE, 'type': 'access'})


def issue_refresh_token(username, client_id):
    return _serializer('mcp-oauth-refresh').dumps(
        {'sub': username, 'client_id': client_id, 'scope': READ_SCOPE, 'type': 'refresh'})


def subject_is_current(username):
    """A grant only counts while it names the app's current login.

    Renaming APP_USERNAME therefore revokes every token issued under the old
    name, the same way rotating MCP_SECRET_KEY revokes all of them.
    """
    current = config.app_username()
    return bool(current) and bool(username) and hmac.compare_digest(username, current)
...
def verify_access_token(token):
    ...
    if payload.get('type') != 'access' or payload.get('scope') != READ_SCOPE:
        return None
    if not subject_is_current(payload.get('sub')):
        return None
    return payload
...
def issue_authorization_code(username, client_id, redirect_uri, code_challenge):
...
APPROVAL_PAGE = """
<!doctype html>
<title>Connect to Bucketlist</title>
...
<div class="card">
  ...
  <ul><li>Bucketlist items</li><li>Scratch map (visited countries)</li>
      <li>Trip plans, dates and prices</li></ul>
...
def authorize():
    ...
    if not check_app_credentials(username, source.get('password', '')):
        record_login_failure()
        return _render_approval(client, redirect_uri, params,
                                error='Invalid username or password', status=401)

    clear_login_failures()
    result = {'code': issue_authorization_code(username, client_id, redirect_uri, code_challenge)}
    if state:
        result['state'] = state
...
def token():
    ...
    if not subject_is_current(username):
        return _token_error('invalid_grant', 'The account for this grant no longer exists')

    response = jsonify({
        'access_token': issue_access_token(username, client_id),
        'token_type': 'Bearer',
